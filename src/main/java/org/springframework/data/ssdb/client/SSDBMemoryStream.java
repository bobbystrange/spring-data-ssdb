...
class SSDBMemoryStream {
    ...
    public SSDBMemoryStream() {
        this(4096);
    }

    public SSDBMemoryStream(int capacity) {
        this.capacity = capacity;
        buf = new byte[capacity];
    }

    public SSDBMemoryStream(byte[] copy) {
        this(copy, 0, copy.length);
    }

    public SSDBMemoryStream(byte[] copy, int off, int len) {
        this.capacity = len;
        this.size = len;
